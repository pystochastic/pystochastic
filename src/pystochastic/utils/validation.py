...
def check_positive_integer(n: int, name: str = "") -> None:
...
def check_numeric(value: float | int, name: str = "") -> None:
    """Ensure that the value is numeric."""
    if not isinstance(value, (float, int)):
        raise TypeError(f"{name} value must be a number.")


def check_positive_number(value: float, name: str = "") -> None:
...
def check_nonnegative_number(value: float, name: str = "") -> None:
...
def check_increments(times: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
...
def times_to_increments(times: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
