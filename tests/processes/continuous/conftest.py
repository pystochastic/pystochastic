...
def mean_func_monotonic(t: float) -> float:
    return t


def mean_func_not_monotonic(t: float) -> float:
    return 1
...
def mean_func(
    request: "FixtureRequest[float | None | callable]",
) -> float | None | callable:
    return request.param
...
def mean_func_invalid(
    request: "FixtureRequest[float | callable | int]",
) -> float | callable | int:
    return request.param


# MultifractionalBrownianMotion
def hurst_const(t: float) -> float:
    return 0.5


def hurst_sin(t: float) -> float:
    return math.sin(t) / 3 + 0.5


@pytest.fixture(params=[None, hurst_const, hurst_sin])
def hurst_func(request: "FixtureRequest[None | callable]") -> None | callable:
    return request.param


def hurst_too_many_args(t: float, u: float) -> float:
    return 0.5


def hurst_out_of_range(t: float) -> float:
    return 1.1


@pytest.fixture(params=[0.5, hurst_too_many_args, hurst_out_of_range])
def hurst_invalid(request: "FixtureRequest[float | callable]") -> float | callable:
    return request.param
...
def rate_func(request: "FixtureRequest[callable]") -> callable:
    return request.param


@pytest.fixture(params=[(1, 100), (1, 10)])
def rate_args(request: "FixtureRequest[tuple]") -> tuple:
    return request.param


@pytest.fixture(params=[{"size": None}])
def rate_kwargs(request: "FixtureRequest[dict]") -> dict:
    return request.param
...
@pytest.fixture(params=[0])
def rate_args_invalid(request: "FixtureRequest[tuple]") -> tuple:
    return request.param


@pytest.fixture(params=[0])
def rate_kwargs_invalid(request: "FixtureRequest[dict]") -> dict:
    return request.param
